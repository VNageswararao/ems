...
class Billing_model extends CI_Model{
 public function __construct()
         ...
         function ajax_call($requestData)
  {
    ...
    $sql = "SELECT billing_master_id,billing_master.invoice_number,CONCAT(fname , '',  lname ,'') AS pateint_name,billing_date,grand_total,modeofpay.name as mode";
 ...
 $print='<button onclick="printdata('.$billing_master_id.')" type="button" class="btn btn-icon btn-info mr-1 mb-1"><i class="la la-print"></i></button>';
  ...
  { 
   ...
   $data['net_amount_in_words']= $this->numberTowords($billing_masters->grand_total);
  ...
  $data['particulars_details']=$this->db->query("select * from billing_detail where billing_master_id=$billing_id")->result();                
         ...
         switch($patient_details->print)
            {
            case 1:$html=$this->load->view("transaction/billing/print",$data, true); 
                   $print_config=[
                                    'format' => 'A4',
                                    'margin_left' => 10,
                                    'margin_right' => 10,
