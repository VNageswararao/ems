                    ...
                    <div class="row match-height">
                        <div class="col-xl-12 col-lg-12">
                            <div class="card">
                                ...
                                <h4 class="card-title">OPD Consultation Charge</h4>
                                </div>
                                <div class="card-content">
                                    <div class="card-body">
                                        ...
                                        <ul class="nav nav-tabs">
                                            <li class="nav-item">
                                                <a class="nav-link active" id="base-tab1" data-toggle="tab" aria-controls="tab1" href="#tab1" aria-expanded="true">Add OPD Consultation Charge</a>
                                            </li>
                                            <li class="nav-item">
                            ...
                            <form id="Opdpanel" action="#" method="post"> 
                                <input type="hidden" name="csrf_test_name" id="csrf_test_name" value="<?php echo $this->security->get_csrf_hash(); ?>"> 
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label for="lastname">Appointment type: <span class="text-danger">*</span></label>
                                             <select class="form-control select2" name="appointment_type_id" id="appointment_type_id">
                                                 <option value="">Select Appointment Type</option>
                                                 <?php if($appointment_types){foreach($appointment_types as $data){ ?>
                                                    <option value="<?php print $data['appointment_type_id']; ?>"><?php echo  $data['name'];  ?></option>
                                               <?php  }} ?>
                                             </select>
                                        </div>
                                    </div>
...
$( document ).ready(function() {
   table= $('#tableid').DataTable( {
          'processing': true,
          'serverSide': true,
          'ajax':'<?=base_url()?>master/Opdpanel/ajax_call',
          'columns': [
             { data: 'slno' },
             { data: 'name' },
             { data: 'amount' },
...
function savedata()
{
    if( $("#name").val()=='') {
         ...
         $.ajax({
            type: "POST",
            url: 'Opdpanel/savedata',
            dataType: "json",
            data: $('#Opdpanel').serialize(),
         ...
         $.ajax({
            ...
            success: function(data){
               ...
               if(data.msg != ''){
                 ...
                 $('.modal-title').html('Edit OPD Consultation Charge');
              } else if(data.error != ''){
                Swal.fire({title:"Warning!",text:""+data.error+"",type:"warning",confirmButtonClass:"btn btn-primary",buttonsStyling:!1});
              } else if(data.error_message) 
              {
